...
class WarApi {
  ...
  constructor() {
    this.warData = fs.existsSync('./data/wardata.json') ? require('../data/wardata.json') : {shard: process.env.SHARD_NAME, warNumber: 0, winner: 'NONE', status: this.WAR_IN_PROGRESS, conquestEndTime: null, conquestStartTime: null}
  ...
  warDataUpdate = () => {
    return this.war()
      .then((data) => {
        if (data) {
          data.shard = process.env.SHARD_NAME
