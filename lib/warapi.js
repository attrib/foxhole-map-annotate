...
class WarApi extends EventEmitter {
  ...
  async loadWarData() {
    // !! Throws
    this._warData = fs.existsSync(path.resolve("data/wardata.json"))
      ? /** @type{WarData} */ (
          JSON.parse(fs.readFileSync(path.resolve("data/wardata.json"), "utf8"))
        )
      : {
    ...
    return await this.war()
      .then((data) => {
        if (data) {
          ...
          fs.writeFile(
            path.resolve("data/wardata.json"),
