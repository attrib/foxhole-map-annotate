...
class WarApi {
  ...
  async loadWarData() {
    this.warData = fs.existsSync(path.resolve('data/wardata.json')) ? JSON.parse(fs.readFileSync(path.resolve('data/wardata.json'), 'utf8')) : {shard: config.config.shard.name, warNumber: 0, winner: 'NONE', status: this.WAR_IN_PROGRESS, conquestEndTime: null, conquestStartTime: null}
    ...
    return this.war()
      .then((data) => {
        if (data) {
          ...
          if (this.warData.warNumber > data.warNumber && this.warData.shard === data.shard) {
            return
          }
          if ((this.warData.shard !== data.shard) || (((this.warData.status === this.WAR_PREPARE && data.warNumber === this.warData.warNumber) || (data.warNumber > this.warData.warNumber)) && data.status === this.WAR_IN_PROGRESS)) {
          ...
          this.warData = data
          fs.writeFile(path.resolve('data/wardata.json'), JSON.stringify(data, null, 2), err => {
            if (err) {
              console.error(err);
            }
          })
