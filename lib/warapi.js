...
class WarApi {
  ...
  constructor() {
    this.warData = fs.existsSync('./data/wardata.json') ? require('../data/wardata.json') : {shard: 'Abel', warNumber: 1, winner: 'NONE', status: this.WAR_IN_PROGRESS, conquestEndTime: null, conquestStartTime: null}
    ...
    return this.war()
      .then((data) => {
        if (data) {
          ...
          console.log(this.warData, data)
          console.log(((this.warData.status === this.WAR_PREPARE && data.warNumber === this.warData.warNumber) || (data.warNumber > this.warData.warNumber)) && data.status === this.WAR_IN_PROGRESS)
          console.log(this.warData.status === this.WAR_IN_PROGRESS && data.status === this.WAR_RESISTANCE)
          console.log((this.warData.status === this.WAR_RESISTANCE || this.warData.status === this.WAR_IN_PROGRESS) && data.status === this.WAR_PREPARE)
