...
module.exports.hasAccess = (userId, userAcl, action, feature = null) => {
  ...
  if (userAcl === ACL_FULL) {
    if (action === ACL_ACTIONS.TRACK_ADD || action === ACL_ACTIONS.ICON_ADD) {
      return true;
    }
  ...
  if (userAcl === ACL_ICONS_ONLY) {
    if (action === ACL_ACTIONS.TRACK_ADD || action === ACL_ACTIONS.TRACK_EDIT || action === ACL_ACTIONS.TRACK_DELETE) {
      return false;
    }
