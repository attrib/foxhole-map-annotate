...
const CONFIG_FILE = process.env.NODE_ENV !== 'test' ? resolve("data/config.yml") : resolve("data/config.test.yml");

// !! Blocking
if (!fs.existsSync(CONFIG_FILE)) {
  const DATA_DIR = dirname(CONFIG_FILE)
  if (!fs.existsSync(DATA_DIR)) {
    fs.mkdirSync(DATA_DIR, { recursive: true });
  }
  fs.copyFileSync(resolve("config.dist.yml"), CONFIG_FILE);
}
class Config {
  /** @type {?ConfigData} */
  _config = null;
 ...
 * @property {ConfigWeather} weather
 */
