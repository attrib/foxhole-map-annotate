...
module.exports = class Discord {
  ...
  checkAllowedUser = async () => {
    ...
    if (userData.id in config.config.access.users) {
      debugInfo.push(`Direct access for user (${config.config.access.users[userData.id]})`)
      return {
    ...
    const highestAccess = []
    const discordsToCheck = this.preferredDiscordServer !== null ? [this.preferredDiscordServer] : Object.keys(config.config.access.roles)
