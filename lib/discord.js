import { ACL_ORDER, ACL_BLOCKED } from "./ACLS.ts";
import config from "./config.js";
import { recomputeMemberships } from "./Groups/groupMemberships.ts";
...
export class Discord {
   ...
   * @param {SessionWithGrant} session
   * @returns {Promise<CheckAllowedRejectedUserObject | CheckAllowedUserObject>}
   */
  checkAllowedUser = async (session) => {
    ...
    if (configUserData !== undefined) {
      ...
      await recomputeMemberships(session, userData.id);
      return {
    ...
    if (highestLevel !== undefined) {
      return {
        access: highestLevel.acl !== ACL_BLOCKED,
 ...
 * @property {import("./ACLS.ts").Access} acl
 * @property {string} discordId
 */

/**
 * Check allowed rejected user object
 * @typedef {object} CheckAllowedRejectedUserObject
 * @property {false} access
 * @property {?string} user
...
/**
 * @typedef {Session & Partial<SessionData>} SessionWithGrant
 */
