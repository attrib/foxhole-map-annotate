...
const extent = /** @type{const} */ ([-2042, 1776]);
 ...
 */
export async function updateMap(noRegen = false) {
  ...
  if (war.features.length === 0 && !noRegen) {
      ...
      for (const item of data.mapItems) {
        if ((warApi.isIconTypeOfType(item.iconType, "town", "industry", "stormCannon") || warApi.isIconTypeOfIcon(item.iconType, "MapIconFacilityMineOilRig")) ) {
          ...
          if (feature) {
            ...
            console.log('Unable to find item in static.json - adding now', region.id, type, item)
            addStaticFeature(region, item)
            regionsSave = true
          }
        }
...
/**
 * @param {RegionFeatureObject} region
 * @param {MapItem} item
 */
function addStaticFeature(region, item) {
  const type = warApi.iconTypes[item.iconType].type
  const icon = warApi.iconTypes[item.iconType].icon
