...
const conquerFileName = process.env.NODE_ENV !== 'test' ? __dirname + '/../data/conquer.json' : __dirname + '/../data/conquer.test.json';
const warFileName = process.env.NODE_ENV !== 'test' ? __dirname + '/../data/war.json' : __dirname + '/../data/war.test.json';
const staticFileName = __dirname + '/../public/static.json';
      ...
      for (const item of data.mapItems) {
        if (item.iconType in warapi.iconTypes && (warapi.iconTypes[item.iconType].type === 'town' || warapi.iconTypes[item.iconType].type === 'industry' || warapi.iconTypes[item.iconType].type === 'stormCannon' || (warapi.iconTypes[item.iconType].type === 'field' && warapi.iconTypes[item.iconType].icon === 'MapIconFacilityMineOilRig')) ) {
          ...
          if (feature) {
            ...
            const id = (foundSC && conquer.features[foundSC].team === team) ? foundSC : randomUUID()
