...
const conquerFileName = process.env.NODE_ENV !== 'test' ? path.resolve('data/conquer.json') :  path.resolve('data/conquer.test.json');
const warFileName = process.env.NODE_ENV !== 'test' ? path.resolve('data/war.json') : path.resolve('data/war.test.json');
const staticFileName = path.resolve('public/static.json');
const conquer = fs.existsSync(conquerFileName) && process.env.NODE_ENV !== 'test' ? JSON.parse(fs.readFileSync(conquerFileName, 'utf8')) : {
  deactivatedRegions: undefined,
  regions: {},
  features: {},
  version: ''
};
const regions = JSON.parse(fs.readFileSync(staticFileName, 'utf8'))
const war = fs.existsSync(warFileName) && process.env.NODE_ENV !== 'test' ? JSON.parse(fs.readFileSync(warFileName, 'utf8')) : {features: [], version: ''};
let blocked = false
      ...
      for (const item of data.mapItems) {
        if (item.iconType in warapi.iconTypes && (warapi.iconTypes[item.iconType].type === 'town' || warapi.iconTypes[item.iconType].type === 'industry' || warapi.iconTypes[item.iconType].type === 'stormCannon' || (warapi.iconTypes[item.iconType].type === 'field' && warapi.iconTypes[item.iconType].icon === 'MapIconFacilityMineOilRig')) ) {
          ...
          if (feature) {
            ...
            const id = (foundSC && conquer.features[foundSC].team === team) ? foundSC : crypto.randomUUID()
            if (!(foundSC && conquer.features[foundSC].team === team)) {
              conquer.features[id] = {
                team,
                icon,
                type,
...
function saveRegions() {
  ...
  fs.writeFileSync(warFileName, json);
  fs.writeFileSync(conquerFileName, JSON.stringify(conquer, null, 2));
}
